 ...
 * @api public
 */
module.exports = function activation(options, activate, deny) {
  ...
  if (!deny) { throw new TypeError('oauth2orize.device.activate grant requires a deny callback'); }
  ...
  function response(txn, res, complete, next) {
    ...
    function denied(err) {
      ...
      respond(txn, res, params);
    }

    function denyDeviceCode() {
      try {
        var denyArity = deny.length;
        if (denyArity == 7) {
          deny(txn.client, txn.locals.deviceCode, txn.user, txn.res, txn.req, txn.locals, denied);
        } else if (denyArity == 6) {
          deny(txn.client, txn.locals.deviceCode, txn.user, txn.res, txn.req, denied);
        } else if (denyArity == 5) {
          deny(txn.client, txn.locals.deviceCode, txn.user, txn.res, denied);
        } else { // arity == 4
          deny(txn.client, txn.locals.deviceCode, txn.user, denied);
        }
      } catch (ex) {
        next(ex);
      }
